...
export default defineDriver((opts = {}) => {
	...
	const getKv = () => {
		...
		if (opts.openKv) {
			...
			if (!globalThis.Deno) {
				throw createError(DRIVER_NAME, "Missing global `Deno`. Are you running in Deno? (hint: use `deno-kv-node` driver for Node.js)");
			}
			if (!Deno.openKv) {
				throw createError(DRIVER_NAME, "Missing `Deno.openKv`. Are you running Deno with --unstable-kv?");
			}
...
function normalizeTTL(ttl) {
	return typeof ttl === "number" && ttl > 0 ? ttl * 1e3 : undefined;
}
