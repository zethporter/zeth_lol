...
export default defineDriver((opts) => {
		...
		if (!_client) {
			...
			if (!opts.url) {
				...
				if (envPrefix && process.env[envName]) {
					...
					throw createError("vercel-kv", `missing required \`url\` option or '${envName}' env.`);
				}
			}
			if (!opts.token) {
				...
				if (envPrefix && process.env[envName]) {
					...
					throw createError("vercel-kv", `missing required \`token\` option or '${envName}' env.`);
				}
			}
	...
	return {
		...
		setItem(key, value, tOptions) {
			...
			return getClient().set(r(key), value, ttl ? { ex: ttl } : undefined).then(() => {});
		},
		removeItem(key) {
