...
import { joinURL } from "./utils/path.mjs";
...
export default defineDriver((opts) => {
	...
	return {
		...
		async getItem(key, tops) {
			const value = await _fetch(r(key), { headers: getHeaders(tops) }).catch(catchFetchError);
			return value;
		},
		...
		async getKeys(base, topts) {
			const value = await _fetch(rBase(base), { headers: getHeaders(topts) });
