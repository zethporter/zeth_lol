...
export default defineDriver((opts) => {
	...
	const getCosmosClient = async () => {
		...
		if (opts.accountKey) {
			...
			const { database } = await cosmosClient.databases.createIfNotExists({ id: opts.databaseName || "unstorage" });
	...
	return {
		...
		async getMeta(key) {
			const item = await (await getCosmosClient()).item(key).read();
			return { mtime: item.resource?.modified ? new Date(item.resource.modified) : undefined };
		},
		async clear() {
			const iterator = (await getCosmosClient()).items.query(`SELECT { id } from c`);
