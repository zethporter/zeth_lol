...
type SealOptions = Readonly<{
  /** Encryption step options. */encryption: SealOptionsSub<EncryptionAlgorithm>; /** Integrity step options. */
  integrity: SealOptionsSub<IntegrityAlgorithm>;
  ttl: number; /** Number of seconds of permitted clock skew for incoming expirations. Defaults to 60 seconds. */
...
type SealOptionsSub<Algorithm extends _Algorithm = _Algorithm> = Readonly<{
  /** The length of the salt (random buffer used to ensure that two identical objects will generate a different encrypted result). Defaults to 256. */saltBits: number; /** The algorithm used. Defaults to 'aes-256-cbc' for encryption and 'sha256' for integrity. */
  algorithm: Algorithm; /** The number of iterations used to derive a key from the password. Defaults to 1. */
  iterations: number; /** Minimum password size. Defaults to 32. */
...
interface TypedResponse<Body = unknown, Headers$1 extends Record<string, string> | unknown = ResponseHeaderMap> extends Omit<Response, 'clone' | 'headers' | 'json'> {
  clone: () => TypedResponse<Body, Headers$1>;
  json: () => Promise<Body>;
  headers: TypedHeaders<Headers$1>;
}
interface TypedRequest<Body = unknown, Headers$1 extends Record<string, string> | unknown = RequestHeaderMap> extends Omit<Request, 'clone' | 'headers' | 'json'> {
  clone: () => TypedRequest<Body, Headers$1>;
  json: () => Promise<Body>;
  headers: TypedHeaders<Headers$1>;
}
...
type H3 = H3$1;
export { FetchableObject as A, SessionData as B, EventHandler as C, EventHandlerResponse as D, EventHandlerRequest as E, TypedServerRequest as F, unsealSession as G, clearSession as H, MaybePromise as I, CookieSerializeOptions as J, updateSession as K, TypedRequest as L, InferEventInput as M, LazyEventHandler as N, EventHandlerWithFetch as O, Middleware as P, Session as R, DynamicEventHandler as S, EventHandlerObject as T, getSession as U, SessionManager as V, sealSession as W, definePlugin as _, H3EventContext as a, ErrorInput as b, H3Plugin as c, HTTPMethod as d, MatchedRoute as f, RouterContext as g, RouteOptions as h, HTTPEvent as i, HTTPHandler as j, FetchHandler$1 as k, H3Route as l, PreparedResponse as m, H3Core as n, H3Config as o, MiddlewareOptions as p, useSession as q, H3Event as r, H3CoreConfig as s, H3 as t, H3RouteMeta as u, ErrorBody as v, EventHandlerFetch as w, HTTPError as x, ErrorDetails as y, SessionConfig as z };