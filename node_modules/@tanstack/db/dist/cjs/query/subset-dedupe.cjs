...
const predicateUtils = require("./predicate-utils.cjs");
  ...
  constructor(opts) {
    ...
    this.loadSubset = (options) => {
      ...
      if (this.unlimitedWhere !== void 0 && options.where !== void 0) {
        if (predicateUtils.isWhereSubset(options.where, this.unlimitedWhere)) {
          this.onDeduplicate?.(options);
          return true;
        }
      }
      if (options.limit !== void 0) {
        const alreadyLoaded = this.limitedCalls.some(
          (loaded) => predicateUtils.isPredicateSubset(options, loaded)
        );
        if (alreadyLoaded) {
          this.onDeduplicate?.(options);
          return true;
      ...
      const matchingInflight = this.inflightCalls.find(
        (inflight) => predicateUtils.isPredicateSubset(options, inflight.options)
      );
      ...
      if (this.unlimitedWhere !== void 0 && options.limit === void 0) {
        clonedOptions.where = predicateUtils.minusWherePredicates(clonedOptions.where, this.unlimitedWhere) ?? clonedOptions.where;
      }
