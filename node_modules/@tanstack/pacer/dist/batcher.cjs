...
var Batcher = class {
	...
	constructor(fn, initialOptions) {
		this.fn = fn;
		this.store = new _tanstack_store.Store(getDefaultBatcherState());
		...
		if (this.key) require_event_client.pacerEventClient.on("d-Batcher", (event) => {
	...
	#setState = (newState) => {
		...
		require_event_client.emitChange("Batcher", this);
	};
	#getWait = () => {
	...
	*/
	#execute = () => {
		...
		const batch$1 = this.peekAllItems();
		this.clear();
		this.options.onItemsChange?.(this);
		this.fn(batch$1);
		this.#setState({
			executionCount: this.store.state.executionCount + 1,
			totalItemsProcessed: this.store.state.totalItemsProcessed + batch$1.length
		});
		this.options.onExecute?.(batch$1, this);
	};
	#clearTimeout = () => {
		if (this.#timeoutId) {
