...
let _tanstack_pacer_async_rate_limiter = require("@tanstack/pacer/async-rate-limiter");

//#region src/async-rate-limiter/useAsyncRateLimiter.ts
/**
...
* ```
*/
function useAsyncRateLimiter(fn, options, selector = () => ({})) {
	const mergedOptions = {
		...require_PacerProvider.useDefaultPacerOptions().asyncRateLimiter,
		...options
	};
	const [asyncRateLimiter] = (0, react.useState)(() => {
		const asyncRateLimiterInstance = new _tanstack_pacer_async_rate_limiter.AsyncRateLimiter(fn, mergedOptions);
		asyncRateLimiterInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useStore)(asyncRateLimiterInstance.store, props.selector);
	...
	const state = (0, _tanstack_react_store.useStore)(asyncRateLimiter.store, selector);
...
//#endregion
exports.useAsyncRateLimiter = useAsyncRateLimiter;
//# sourceMappingURL=useAsyncRateLimiter.cjs.map