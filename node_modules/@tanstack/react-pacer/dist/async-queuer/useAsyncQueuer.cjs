...
let _tanstack_pacer_async_queuer = require("@tanstack/pacer/async-queuer");

//#region src/async-queuer/useAsyncQueuer.ts
/**
...
* ```
*/
function useAsyncQueuer(fn, options = {}, selector = () => ({})) {
	const mergedOptions = {
		...require_PacerProvider.useDefaultPacerOptions().asyncQueuer,
		...options
	};
	const [asyncQueuer] = (0, react.useState)(() => {
		const asyncQueuerInstance = new _tanstack_pacer_async_queuer.AsyncQueuer(fn, mergedOptions);
		asyncQueuerInstance.Subscribe = function Subscribe(props) {
			const selected = (0, _tanstack_react_store.useStore)(asyncQueuerInstance.store, props.selector);
	...
	const state = (0, _tanstack_react_store.useStore)(asyncQueuer.store, selector);
...
//#endregion
exports.useAsyncQueuer = useAsyncQueuer;
//# sourceMappingURL=useAsyncQueuer.cjs.map